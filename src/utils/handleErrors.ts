...
const pluck = (err: any): Record<string, any> => {
 ...
 */
export const ERROR_MESSAGE_OVERRIDES: Record<string, typeof pluck> = {
  42501: err => ({
    ...pluck(err),
...
function conflictFieldsFromError(err: any) {
  ...
  if (constraint && table) {
    ...
    if (constraint.startsWith(PREFIX)) {
      ...
      if (matchingSuffix) {
        const maybeColumnNames = constraint.substring(
          PREFIX.length,
          constraint.length - matchingSuffix.length,
        )
...
export default function handleErrors(
  ...
  return errors.map((error) => {
    ...
    const code = originalError ? get(originalError, ['code']) : null
